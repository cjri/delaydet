...
void PrintSigEpi (double beta, vector< vector<double> >& sigs, vector<epi>& epistat2, vector<epi>& epistat3, vector<epi>& epistat4, vector<epi>& epistat5) {
	cout << "New sigma\n";
	for	(int i=0;i<sigs.size();i++) {
		cout << i << " ";
		for (int j=0;j<sigs[i].size();j++) {
...
void Propagation(int verb, double beta, vector<int> times, vector<double> init_freqs, vector< vector<double> > mut, vector< vector<double> > hapsigs, vector< vector<double> >& inf) {
...
void GrowGeneration(double beta, vector<vector<double> >& m, vector<double>& q, vector<double>& sigma) {
	vector<double> temp;
	MatMult(beta,m,q,temp);  //Now using m^2
	q=temp;
...
void MatMult (double beta, vector<vector<double> >& m, vector<double>& v, vector<double>& mv) {
	mv.clear();
	mv.assign(v.size(),0);

...
int main(int argc, const char **argv) {
	...
	for (int rep=0;rep<maxrep;rep++) {
		...
		for (int it=0;it<p.iterations;it++) {
			...
			if (first==0) {
				...
				if (dL>0) {
					...
					if ((it>p.iterations-10000)||(it%20==1)) {
						...
						PrintSigEpi (sigs,epistat2,epistat3,epistat4,epistat5);
						cout << "System\n";
						Propagation(1,times2,init_freqs1,mut,hapsigs,inf2);
						cout << "Better L = " << L << "\n";
			...
			Propagation(0,times2,init_freqs1,mut,hapsigs,inf2);
